...
export default function ExportacaoPage() {
  ...
  }, [siloId, barras, barrasSelecionadas, grandezas]);
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      const res = await fetch(`${baseUrl}/export/${tipoLeitura}?${params.toString()}`, {
        headers: { Authorization: `Bearer ${token ?? ''}` },
      });

      if (!res.ok) {
        const err = await res.json().catch(() => ({ message: 'Erro na exportação' })) as { message?: string };
      ...
      const filename = disposition?.match(/filename=(.+)/)?.[1] ?? `${tipoLeitura}_${Date.now()}.${ext}`;
  ...
  const canExport = siloId !== null && grandezas.length > 0 && sensores.length > 0;
